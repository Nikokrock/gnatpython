"""Modification of the yaml loader for gnatpython."""


from collections import OrderedDict
from gnatpython.stringutils import format_with_dict
import logging
import os
import re
import sys
...
logger = logging.getLogger("gnatpython.internal.yaml_utils")
...
class CaseParser(object):
    ...
    def __parse_case(self, case_key, data):
        ...
        if result is not None:
            logger.debug("%s=%s match %s" % (key, result[0], result[1]))
            return result[2]
        else:
            return None
    ...
    def __format_value(self, value):
        ...
        try:
            ...
            logger.debug("Cannot format %s, ignore it", value)
    ...
    def __update_state(self, key, value, cursor, prefix):
        ...
        if real_key not in cursor or real_key == key:
            logger.debug('set %s -> %s' % (real_key_str, real_value))
            cursor[real_key] = real_value
        else:
            if isinstance(cursor[real_key], dict):
                logger.debug('update %s -> %s' % (real_key_str, real_value))
...
def load_with_config(filename, config):
    ...
    for f in filename:
        try:
            logger.debug('load config file: %s' % f)
...
def load_with_regexp_table(filename, selectors, data):
    ...
    elements.
    """
    logging.debug('load %s with %s', filename, selectors)
    with open(filename) as f:
    ...
    logger.debug('yaml results: %s', result)
    return result
